...
export function registerPRTools(server) {
    server.registerTool("get_user_prs", {
        title: "Get User Pull Requests",
        description: "Get pull request statistics for a specific user in a GitHub organization",
        ...
        if (until)
            query += ` created:<=${until}`;
        const searchResults = await octokit.paginate("GET /search/issues", {
            q: query,
            per_page: 100,
